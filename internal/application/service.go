...
func New(services Services) *Service {
	return &Service{store: services.Store, archive: services.Archive, boxes: newMailboxes(32), views: make(map[string]CaseView)}
}
...
func (s *Service) Get(caseID string) (CaseView, error) {
	s.viewMu.RLock()
	if cached, ok := s.views[caseID]; ok {
		s.viewMu.RUnlock()
		return cloneCaseView(cached), nil
	}
	s.viewMu.RUnlock()
	...
	s.viewMu.Lock()
	s.views[caseID] = cloneCaseView(v)
	s.viewMu.Unlock()
	return v, nil
}

func cloneCaseView(value CaseView) CaseView {
