	...
	for {
		...
		if len(line) > 0 {
			...
			if digestBytes(canonical) != digest {
				// BUG(seed): 将可分类的数据损坏错误格式化为普通字符串，跨层后无法映射 CodeCorrupt。
				corrupt := &domain.RuleError{Code: domain.CodeCorrupt, Message: "事件摘要校验失败"}
				return nil, fmt.Errorf("事件摘要校验失败: %v", corrupt)
			}
